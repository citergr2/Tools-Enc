...
# ========================
# Auto Compile Shell Script ke Binary
# ========================
auto_compile() {
  clear
  figlet "AUTO COMPILE"
  echo ""
  read -p "Masukkan path file .sh: " script_file
  if [[ ! -f "$script_file" ]]; then
    color red "File tidak ditemukan!"
    return
  fi

  echo -e "${CYAN}Pilih target arsitektur:${NC}"
  echo -e "${CYAN}1. Native (x86_64 Linux)${NC}"
  echo -e "${CYAN}2. Android ARM (Termux)${NC}"
  echo -e "${CYAN}3. Android ARM64${NC}"
  echo ""
  read -p "Pilihan (1/2/3): " target

  output="${script_file%.sh}"
  case "$target" in
    1)
      shc -f "$script_file" -o "$output" && color green "✓ Binary berhasil dibuat: $output"
      ;;
    2)
      if command -v aarch64-linux-gnu-gcc >/dev/null 2>&1; then
        shc -f "$script_file" -o "${output}_arm"
        aarch64-linux-gnu-strip "${output}_arm"
        color green "✓ ARM binary berhasil: ${output}_arm"
      else
        color red "Cross-compiler ARM (aarch64-linux-gnu-gcc) tidak ditemukan!"
      fi
      ;;
    3)
      if command -v arm-linux-gnueabi-gcc >/dev/null 2>&1; then
        shc -f "$script_file" -o "${output}_arm32"
        arm-linux-gnueabi-strip "${output}_arm32"
        color green "✓ ARM32 binary berhasil: ${output}_arm32"
      else
        color red "Cross-compiler ARM32 (arm-linux-gnueabi-gcc) tidak ditemukan!"
      fi
      ;;
    *)
      color red "Pilihan tidak valid!"
...
# ========================
# Auto Compile to AARCH64
# ========================
auto_compile_aarch64() {
  clear
  figlet "COMPILE AARCH64"
  echo ""
  read -p "Masukkan path file .sh: " script_file
  ...
  if ! command -v shc >/dev/null 2>&1; then
    color red "shc belum terpasang!"
    return
  fi

  if ! command -v aarch64-linux-gnu-gcc >/dev/null 2>&1; then
    color red "Cross-compiler aarch64-linux-gnu-gcc belum terpasang!"
    return
  fi

  output_name="${script_file%.sh}_aarch64"

  # Compile pakai shc
  shc -f "$script_file" -o temp_aarch64
  if [[ ! -f temp_aarch64.x.c ]]; then
    color red "Gagal menghasilkan file .c dari shc"
    return
  fi

  # Compile pakai cross-compiler
  aarch64-linux-gnu-gcc temp_aarch64.x.c -o "$output_name" && \
    color green "✓ Berhasil: Binary AARCH64 -> $output_name" || \
    color red "✗ Gagal saat compile dengan aarch64-linux-gnu-gcc"

  # Cleanup
  rm -f temp_aarch64.x.c
}
...
while true; do
  ...
  echo -e "${CYAN}18.AUTO COMPILE${NC}"
  echo -e "${CYAN}19.COMPILE TO AARCH64 (SHC)${NC}"

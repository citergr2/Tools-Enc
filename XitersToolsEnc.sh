...
if [[ "$PREFIX" != "/data/data/com.termux/files/usr" ]]; then
  echo "❌ Skrip ini hanya disarankan dijalankan di Termux."
  exit 1
fi

# Fungsi untuk menyarankan instalasi
suggest_install() {
  case "$1" in
    figlet) echo "pkg install figlet" ;;
    qrencode) echo "pkg install qrencode" ;;
    yt-dlp) echo "pip install yt-dlp" ;;
    wget) echo "pkg install wget" ;;
    curl) echo "pkg install curl" ;;
    redis-cli) echo "pkg install redis" ;;
    shc) echo "pkg install shc" ;;
    *) echo "Silakan install $1 secara manual" ;;
  esac
}
...
for cmd in figlet qrencode yt-dlp wget curl redis-cli; do
  if ! command -v "$cmd" >/dev/null 2>&1; then
    echo "❌ '$cmd' tidak ditemukan. Coba install dengan:"
    suggest_install "$cmd"
    exit 1
  fi
done
